 ...
 */
object TestDataAlternatingGenerator extends Worker {
  ...
  val workingDirectory = "data/generated/alternating/"
    ...
    def getTrainingSeq: Seq[Seq[Double]] = {
      (0 until seqLength).map { position =>
        if(position % 2 != 0) // demonstrate attention
          Seq.fill(768) {withOptionalNoiseAndScaling(seqLength - position)} // with an alternating timeseries
